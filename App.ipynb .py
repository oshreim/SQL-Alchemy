...
def precipitation():
  ...
  prcp_scores = (session.query(Measurement.date, Measurement.prcp).      filter(Measurement.date >= year_ago).      order_by(Measurement.date).      all())# Query for the date and precipitation for the last year
...
def temp_monthly():
  ...
  temp_freq= session.query(Measurement.tobs).                              filter(Measurement.station=='USC00519281').                              filter(Measurement.date >= year_ago).                              all()
